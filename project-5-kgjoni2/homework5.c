/*
 I COMMENTED OUT THE CODE I ADDED 
 BECAUSE I GOT A SEG FAULT AND I 
...
void serve_request(int client_fd){
  ...
  requested_file = strstr(filename, filename[0]);
    
  int x = strcmp(filename, "public_html/");
  int error;    
...
int main(int argc, char** argv) {
    ...
    */    
    ...
    for (i = 0; i < num_threads; i++) {
        ...
        strcpy(arguments[i].name, names[argv[i]]);

        int retval = pthread_create(&threads[i], NULL, thread_function, (void *) &arguments[i]);
        if (retval) {
